...
“设置 → 通知 → 自动确认模式仍通知”默认**开启**；关闭后，只会屏蔽已明确以完全跳过确认方式启动的实例的 `等待确认` 通知及其后续提醒，不会隐藏 `等待回复` 通知。该项只会影响**当前已处于自动确认模式**的会话：Codex 从其每回合写入的运行时 `approval_policy` 判断，只有值为 `never` 才会被识别为自动确认；`on-request` 等其他策略仍会照常发送等待确认通知。Claude、OpenCode 和 DeepSeek Harness 暂未在可稳定读取的会话元数据中提供等价状态，因此仍会照常通知。

...
| 人工介入提醒 | 可在“设置 → 通知”启用；等待确认或等待回复时立即通知，持续 60 秒再次提醒，持续 3 分钟发送最后提醒，点击通知可跳转对应会话；可单独隐藏完全自动确认模式中的等待确认提醒 |
...
“设置 → 通知 → 自动确认模式仍通知”默认**开启**；关闭后，只会屏蔽已明确以完全跳过确认方式启动的实例的 `等待确认` 通知及其后续提醒，不会隐藏 `等待回复` 通知。该项只会影响**当前已处于自动确认模式**的会话。Codex 从每回合写入的运行时元数据判断：`approval_policy: never`，或 `approvals_reviewer: auto_review`（`--approve-for-me` / 自动审核），才会被识别为自动确认；`approval_policy: on-request` 且 `approvals_reviewer: user` 等人工确认策略仍会照常发送等待确认通知。Claude、OpenCode 和 DeepSeek Harness 暂未在可稳定读取的会话元数据中提供等价状态，因此仍会照常通知。
...
| 自动确认模式的等待确认提醒 | 点击“设置 → 通知 → 自动确认模式仍通知”切换；默认通知；关闭时仅屏蔽当前处于 Codex 自动确认模式的等待确认通知 |
...
显示配置、通知开关、自动确认模式下的等待确认提醒开关和浏览器标签页复用开关保存在 `~/.config/agent-statusbar/config.json`。开机自启状态由 `~/Library/LaunchAgents/com.agentstatusbar.monitor.plist` 判断；开启时安装并启动服务，关闭时卸载服务并删除 plist。首次运行时五项显示内容默认开启，通知和浏览器标签页复用默认关闭。受 macOS 原生菜单行为限制，执行操作后菜单会关闭；重新打开即可查看最新状态。

...
通知开启后，“设置 → 通知 → 通知选项”提供三个默认开启、可多选的提醒类型：`等待确认`、`等待回复` 和 `自动确认模式仍通知`。关闭“等待确认”会屏蔽所有等待确认提醒；关闭“等待回复”只屏蔽等待回复提醒；关闭“自动确认模式仍通知”则只屏蔽自动确认会话的等待确认提醒，不影响普通人工确认会话和等待回复。Codex 从每回合写入的运行时元数据判断自动确认：`approval_policy: never`，或 `approvals_reviewer: auto_review`（`--approve-for-me` / 自动审核）。`approval_policy: on-request` 且 `approvals_reviewer: user` 等人工确认策略仍属于普通等待确认。Claude、OpenCode 和 DeepSeek Harness 暂未在可稳定读取的会话元数据中提供等价状态，因此仍归入普通等待确认。

界面和通知语言默认读取 macOS 的“语言与地区”设置。也可以在菜单栏“设置 → 语言”中选择“跟随系统”、English、简体中文或繁體中文；手动选择会保存到 `~/.config/agent-statusbar/config.json`。系统语言模式下会依次匹配首选语言，未匹配时按地区选择中文变体，仍无法匹配时使用英语。语言缓存每 60 秒刷新，更改系统语言后无需重启，最多约 60 秒自动生效。
...
| 配置通知类型 | 先开启通知，再进入“设置 → 通知 → 通知选项”；可多选等待确认、等待回复和自动确认模式仍通知 |
...
| 切换界面语言 | 点击“设置 → 语言”，选择跟随系统、English、简体中文或繁體中文；切换后会自动重启监控守护进程 |
| 调整显示内容 | 悬浮“设置 → 显示配置”，点击子菜单选项即可切换 |

显示配置、通知开关、各类通知选项和浏览器标签页复用开关保存在 `~/.config/agent-statusbar/config.json`。开机自启状态由 `~/Library/LaunchAgents/com.agentstatusbar.monitor.plist` 判断；开启时安装并启动服务，关闭时卸载服务并删除 plist。首次运行时六项显示内容（包括已停止 Agent）默认开启，通知和浏览器标签页复用默认关闭。受 macOS 原生菜单行为限制，执行操作后菜单会关闭；重新打开即可查看最新状态。

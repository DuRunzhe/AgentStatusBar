...
exec "$PYTHON_CMD" "$RENDER_PATH" \
  "$STATUS_FILE" "$FOCUS_PATH" "$FOCUS_WEB_PATH" "$FOCUS_CODEX_PATH" "$NODE_CMD" "$RESTART_PATH" \
  "$DISPLAY_CONFIG_PATH" "$NOTIFICATION_SETTINGS_PATH" "$BROWSER_TAB_SETTINGS_PATH" "$STARTUP_SETTINGS_PATH"
